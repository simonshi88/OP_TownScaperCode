...
public class CreateGrid_Relax : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        StartCoroutine(RelaxVectices());
    }


    private IEnumerator RelaxVectices()
    {
        for (int i = 0; i < 500; i++)
        {
            ...
            foreach (var vertex in vertices)
            {
                ...
                Vector3 positionToChange = new Vector3();

                foreach (var edge in vertex.Edges)
                {
                    Vertex directionEdge;

                    if (edge.Vertices[0] == vertex)
                        directionEdge = edge.Vertices[1];
                    else
                        directionEdge = edge.Vertices[0];
            ...
            yield return StartCoroutine(LagOfAnimation(0.001f));
    ...
    private void GenerateVertices()
    {
        ...
        for (var ringIndex = 0; ringIndex < ringCount; ringIndex++)
        {
            ...
            // hex has 6 edges, each edge 2 vertex plus vertices between them depends on ring index
             AddRingEdge(new Vector3(0, newOuterRadius), new Vector3(newInnerRadius, 0.5f * newOuterRadius), ringIndex);
             AddRingEdge(new Vector3(newInnerRadius, 0.5f * newOuterRadius), new Vector3(newInnerRadius, -0.5f * newOuterRadius), ringIndex);
             AddRingEdge(new Vector3(newInnerRadius, -0.5f * newOuterRadius), new Vector3(0, -newOuterRadius), ringIndex);
             AddRingEdge(new Vector3(0, -newOuterRadius), new Vector3(-newInnerRadius, -0.5f * newOuterRadius), ringIndex);
             AddRingEdge(new Vector3(-newInnerRadius, -0.5f * newOuterRadius), new Vector3(-newInnerRadius, 0.5f * newOuterRadius), ringIndex);
             AddRingEdge(new Vector3(-newInnerRadius, 0.5f * newOuterRadius), new Vector3(0, newOuterRadius), ringIndex);
        }
